...
type Model = 'user'|'profile'|'website'|'template'|'gallery'|'music'|'video'|'timeline'|'memory'|'wishlistItem'|'guestbook'|'analyticsEvent'|'media'|'notification'|'emailTemplate'|'setting'|'demoSite';
const tables: Record<Model,string> = {
  ...
  media:'media', notification:'notifications', emailTemplate:'email_templates', setting:'settings', demoSite:'demo_sites'
};
const columns: Record<string,string> = { user:'*', profile:'*', website:'*', template:'*', gallery:'*', music:'*', video:'*', timeline:'*', memory:'*', wishlistItem:'*', guestbook:'*', analyticsEvent:'*', media:'*', notification:'*', emailTemplate:'*', setting:'*', demoSite:'*' };
