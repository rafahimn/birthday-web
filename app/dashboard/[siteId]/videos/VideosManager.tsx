...
export default function VideosManager({ siteId, videos }: { siteId: string; videos: VideoItem[] }) {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <div className="space-y-4">
        {videos.map((v) => (
          <VideoRow key={v.id} siteId={siteId} video={v} />
        ))}
...
function VideoRow({ siteId, video }: { siteId: string; video: VideoItem }) {
  const [pending, startTransition] = useTransition();
    ...
    return (
      <div className="flex items-center justify-between gap-4 rounded-2xl bg-white p-4 shadow-sm">
        ...
        <div className="flex shrink-0 gap-2">
          ...
          <button onClick={() => startTransition(() => deleteVideo(siteId, video.id))} className="btn-danger">
            Delete
          </button>
        </div>
      </div>
  ...
  return (
    <form
      action={(fd) => {
        ...
        setSaved(false);
        startTransition(async () => {
          await updateVideo(siteId, video.id, fd);
          setSaved(true);
          setEditing(false);
        });
      }}
