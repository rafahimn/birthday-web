...
export default function ReasonsManager({ siteId, reasons }: { siteId: string; reasons: Reason[] }) {
  const [pending, startTransition] = useTransition();

  return (
    <div className="max-w-2xl space-y-6">
      <form
        action={(fd) => startTransition(async () => { await addReason(siteId, fd); (document.getElementById("reason-form") as HTMLFormElement)?.reset(); })}
        id="reason-form"
      ...
      <div className="space-y-3">
        {reasons.map((r) => (
          <ReasonRow key={r.id} siteId={siteId} reason={r} />
        ))}
...
function ReasonRow({ siteId, reason }: { siteId: string; reason: Reason }) {
  const [pending, startTransition] = useTransition();

  return (
    <form
      action={(fd) => startTransition(() => updateReason(siteId, reason.id, fd))}
    ...
    >
      ...
      <div className="flex gap-2">
        ...
        <button
          type="button"
          onClick={() => startTransition(() => deleteReason(siteId, reason.id))}
