...
export async function GET(req:Request){
 ...
 const response=NextResponse.redirect(new URL(dest,u.origin));setSessionCookie(response,d.access_token,d.refresh_token);response.cookies.set(googleVerifierCookie,'',{httpOnly:true,path:'/',maxAge:0});return response;
}
